namespace Day16Puzzle02
{
	internal class Node
	{
		...
		private Dictionary<Direction, long> _visits =
			new Dictionary<Direction, long>()
			{
				[Direction.North] = long.MaxValue,
				[Direction.East] = long.MaxValue,
				[Direction.South] = long.MaxValue,
				[Direction.West] = long.MaxValue,
			};
		public bool VisitedFrom(Direction direction, long cost) => _visits[direction] < cost;

		public void SetVisitedFrom(Direction direction, long cost) => _visits[direction] = cost;
		...
		private void MergePath(Path path1, Path path2, long turncost)
		{
			...
			path1.NumPathSegments += path2.NumPathSegments + 1;
		}
