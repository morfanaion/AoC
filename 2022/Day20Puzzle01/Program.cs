...
List<Number> numbers = File.ReadAllLines("Input.txt").Select(line => (previous = new Number(int.Parse(line), previous))).ToList();

foreach (Number number in numbers)
{
    int distance = number.Value % (numbers.Count - 1);
    if (distance != 0)
    {
