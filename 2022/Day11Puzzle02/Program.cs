...
Monkey.Threshold = Monkey.MonkeyList.Product(monkey => monkey.TestDivisor);
for(int i = Monkey.Threshold; i > 0; i--)
{
    if(Monkey.MonkeyList.All(monkey => i % monkey.TestDivisor == 0))
    {
        Monkey.Threshold = i;
    }
}
...
long[] twoHighest = Monkey.MonkeyList.OrderByDescending(monkey => monkey.NumInspections).Take(2).Select(monkey => monkey.NumInspections).ToArray();
