...
Node startNode = grid.SelectMany(row => row).Single(node => node.IsEndPoint);
...
Console.WriteLine(grid.SelectMany(row => row).Where(node => node.Elevation == 0 && node.NumStepsRequired != -1).Min(node => node.NumStepsRequired));
