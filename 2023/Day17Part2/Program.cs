...
/*Step testStep = new Step() { CurrentBlock = cityBlocks[30][30] };
for(int i = 0; i < 9; i ++)
{
    testStep = testStep.NextStep(cityBlocks[28][28]);
}
testStep = testStep.NextStep(testStep.Last9th);

Test(testStep, cb => cb.South ?? throw new Exception("Blaaaah"));
return -1;*/
Step startStep = new Step() { CurrentBlock = start };
for (int i = 0; i < 9; i++)
{
    startStep = startStep.NextStep(cityBlocks[10][10]);
}
startStep = startStep.NextStep(startStep.Last9th);
...
int Calculate(Step start, Func<Step, IEnumerable<(Step s, int weight)>> getNeighbours)
{
    ...
    while (queue.Count > 0)
    {
        ...
        {
            Step tempStep = step;
            while(!tempStep.CurrentBlock.IsStart)
            {
                char direction = '\0';
                if (tempStep.CurrentBlock.X < tempStep.Last.X)
                {
                    tempStep.CurrentBlock.Path = '<';
                }
                else if (tempStep.CurrentBlock.X > tempStep.Last.X)
                {
                    tempStep.CurrentBlock.Path = '>';

                }
                else if (tempStep.CurrentBlock.Y < tempStep.Last.Y)
                {
                    tempStep.CurrentBlock.Path = '^';
                ...
                {
                    tempStep.CurrentBlock.Path = 'V';

                }
                tempStep = distances[tempStep].s;
            }
            for(int y = 0; y < cityBlocks.Count; y++)
            {
                Console.WriteLine(string.Join("", cityBlocks[y].Select(c => c.Path)));
            }
...
void Test(Step step, Func<CityBlock, CityBlock> getFront)
{
    for(int i = 0; i < 12; i++)
    {
        Console.WriteLine($"{StepToString(step)}: {string.Join(", ", step.CurrentBlock.GetNeighbours(step).Select(sw => StepToString(sw.s)))}");
        step = step.NextStep(getFront(step.CurrentBlock));
    }
}

string StepToString(Step step) => $"{step.CurrentBlock.X}|{step.CurrentBlock.Y}";